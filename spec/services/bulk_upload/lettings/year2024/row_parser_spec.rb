      ...
      describe "valid/invalid attributes" do
        ...
        context "when valid row" do
          ...
          it "instantiates a log with everything completed", :aggregate_failures do
            skip "Doesn't seem to work. The 2024 parser isn't used any more so not worth fixing"
      ...
      context "when other charges are not given" do
        ...
        context "and it is not carehome" do
          ...
          it "adds an error to all missing charges" do
            ...
            expect(parser.errors[:field_128]).to eql([I18n.t("validations.lettings.2024.bulk_upload.charges.related_to_missing_charge")])
          end
        end
      end
      ...
      context "when supscharg is not given" do
        context "and it is not carehome" do
          ...
          it "adds an error to all charges" do
            parser.valid?
            expect(parser.errors[:field_125]).to eql([I18n.t("validations.lettings.2024.bulk_upload.charges.related_to_missing_charge")])
            expect(parser.errors[:field_126]).to eql([I18n.t("validations.lettings.2024.bulk_upload.charges.related_to_missing_charge")])
            expect(parser.errors[:field_127]).to eql([I18n.t("validations.lettings.2024.bulk_upload.charges.related_to_missing_charge")])
