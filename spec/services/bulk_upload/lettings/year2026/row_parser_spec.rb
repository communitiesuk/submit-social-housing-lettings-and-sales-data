...
RSpec.describe BulkUpload::Lettings::Year2026::RowParser do
  ...
  describe "validations" do
      ...
      describe "#validate_nulls" do
        context "when non-setup questions are null" do
          ...
          it "fetches the question's check_answer_label if it exists" do
            parser.valid?
            expect(parser.errors[:field_42]).to eql([I18n.t("validations.lettings.2026.bulk_upload.not_answered", question: "lead tenant’s gender identity.")])
          end
        end
    ...
    describe "#field_115" do # referral
      context "when 3 ie PRP nominated by LA and owning org is LA" do
        let(:attributes) { { bulk_upload:, field_115: "3", field_1: owning_org.old_visible_id, field_2: owning_org.old_visible_id } }
      ...
      context "when 4 ie referred by LA and is general needs and owning org is LA" do
        let(:attributes) { { bulk_upload:, field_115: "4", field_1: owning_org.old_visible_id, field_2: owning_org.old_visible_id, field_4: "1" } }
      ...
      context "when 4 ie referred by LA and is general needs and owning org is PRP" do
        ...
        let(:attributes) { { bulk_upload:, field_115: "4", field_1: owning_org.old_visible_id, field_2: owning_org.old_visible_id } }
      ...
      context "when 4 ie referred by LA and is not general needs" do
        ...
        let(:attributes) { { bulk_upload:, field_115: "4", field_4: "2" } }
    ...
    describe "soft validations" do
      ...
      context "when a soft validation is triggered that relates both to fields that are and are not routed to" do
        let(:attributes) { setup_section_params.merge({ field_77: "1", field_42: "M", field_48: "M", field_52: "M" }) }

        it "adds errors to fields that are routed to" do
          parser.valid?
          expect(parser.errors.where(:field_48, category: :soft_validation)).to be_present
          expect(parser.errors.where(:field_52, category: :soft_validation)).to be_present
        end

        it "does not add errors to fields that are not routed to" do
          parser.valid?
          expect(parser.errors.where(:field_56, category: :soft_validation)).not_to be_present
          expect(parser.errors.where(:field_60, category: :soft_validation)).not_to be_present
        end
      end
