...
RSpec.describe Csv::SalesLogCsvService do
  ...
  context "when stubbing :ordered_questions_for_year" do
    ...
    context "when it returns questions with particular ids" do
      ...
      it "includes log attributes related to questions to the headers" do
        attribute_headers_before_2023 = csv.first
        expect(attribute_headers_before_2023).to include(*%w[TYPE AGE1 LIVEINBUYER1])
      end
      ...
      it "removes some log attributes related to questions from the headers and replaces them with their derived values in the correct order" do
        attribute_headers_before_2023 = csv.first
        expect(attribute_headers_before_2023).not_to include "EXDATE"
        expect(attribute_headers_before_2023.last(4)).to eq %w[LIVEINBUYER1 EXDAY EXMONTH EXYEAR]
      end
    end
    ...
    context "when it returns questions with particular features" do
      ...
      it "does not add questions for checks, whether some other attribute is known or whether something else was asked" do
        attribute_headers_before_2023 = csv.first
        expect(attribute_headers_before_2023).not_to include "attribute_value_check"
        expect(attribute_headers_before_2023).not_to include "something_or_other_known"
        expect(attribute_headers_before_2023).not_to include "whatchamacallit_asked"
      end

      it "does not add the id of checkbox questions, but adds the related attributes of the log in the correct order" do
        attribute_headers_before_2023 = csv.first
        expect(attribute_headers_before_2023.last(4)).to eq %w[OWNERSHIP PREGYRHA PREGOTHER TYPE]
      end
    end
  end

  it "includes attributes not related to questions to the headers" do
    expect(attribute_headers).to include(*%w[ID STATUS CREATEDDATE UPLOADDATE])
  end
  ...
  context "when the user is not a support user" do
    ...
    it "does not include certain attributes in the headers" do
      expect(attribute_headers).not_to include(*%w[address_line1_as_entered address_line2_as_entered town_or_city_as_entered county_as_entered postcode_full_as_entered la_as_entered created_by value_value_check monthly_charges_value_check])
    end
