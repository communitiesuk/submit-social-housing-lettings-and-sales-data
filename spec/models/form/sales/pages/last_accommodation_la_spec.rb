...
RSpec.describe Form::Sales::Pages::LastAccommodationLa, type: :model do
  ...
  let(:form) { instance_double(Form, depends_on_met: true, start_date: current_collection_start_date) }
  ...
  it "is routed to for non-discounted sale logs" do
    log.update!(ownershipsch: 1)
    expect(page).to be_routed_to(log, nil)
  end

  it "is not routed to for discounted sale logs" do
