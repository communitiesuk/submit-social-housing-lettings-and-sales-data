...
RSpec.describe Log, type: :model do
  ...
  describe "#calculate_status" do
    it "returns the correct status for a completed sales log" do
      complete_sales_log = build(:sales_log, :completed, saledate: Time.zone.local(2023, 12, 12), status: nil, assigned_to: create(:user))
      expect(complete_sales_log.calculate_status).to eq "completed"
    end

    it "returns the correct status for an in progress sales log" do
      in_progress_sales_log = build(:sales_log, :in_progress, saledate: Time.zone.local(2023, 12, 12), status: nil)
    ...
    it "returns the correct status for an in progress lettings log" do
      in_progress_lettings_log = build(:lettings_log, :in_progress, startdate: Time.zone.local(2023, 12, 12), status: nil)
