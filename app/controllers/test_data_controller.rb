class TestDataController < ApplicationController
  ...
  %w[2024 2025].each do |year|
    define_method("create_#{year}_test_lettings_bulk_upload") do
      ...
      log = FactoryBot.create(:lettings_log, :completed, assigned_to: current_user, ppostcode_full: "SW1A 1AA", startdate: generate_different_date_within_collection_year(Time.zone.local(year.to_i, 4, 1), end_date_override: start_date + 14.days))
