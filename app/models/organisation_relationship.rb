class OrganisationRelationship < ApplicationRecord
  ...
  validate :validate_housing_provider_relationship
  validate :validate_managing_agent_relationship

private

  def validate_housing_provider_relationship
    if parent_organisation_id.blank?
      errors.add :parent_organisation_id, "You must choose a housing provider"
    elsif OrganisationRelationship.exists?(child_organisation:, parent_organisation:)
      errors.add :parent_organisation_id, "You have already added this housing provider"
  ...
  def validate_managing_agent_relationship
    if child_organisation_id.blank?
      errors.add :child_organisation_id, "You must choose a managing agent"
    elsif OrganisationRelationship.exists?(child_organisation:, parent_organisation:)
      errors.add :child_organisation_id, "You have already added this managing agent"
    end
  end
end
