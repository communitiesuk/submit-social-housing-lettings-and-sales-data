  ...
  scope :duplicates, lambda { |created_by_id = nil|
    ...
    if created_by_id
      scope = scope.where("EXISTS (
                            SELECT 1
                            FROM lettings_logs AS inner_logs
                            WHERE lettings_logs.tenancycode = inner_logs.tenancycode
                              AND lettings_logs.startdate = inner_logs.startdate
                              AND lettings_logs.age1_known = inner_logs.age1_known
                              AND lettings_logs.age1 = inner_logs.age1
                              AND lettings_logs.sex1 = inner_logs.sex1
                              AND lettings_logs.ecstat1 = inner_logs.ecstat1
                              AND lettings_logs.tcharge = inner_logs.tcharge
