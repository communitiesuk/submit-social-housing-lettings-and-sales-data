module Csv
  class LettingsLogCsvService
    ...
    def to_csv
      CSV.generate(headers: true) do |csv|
        ...
        LettingsLog.all.find_each do |log|
          csv << @attributes.map { |attribute| value(attribute, log) }
        end
      end
    end
  ...
  private
    ...
    def lettings_log_attributes
      ...
      # ordered_questions.reject! { |q| q.id.match?(/(_known)|(_check)/) }
      ...
      scheme_attributes = %w[scheme_code scheme_service_name scheme_sensitive SCHTYPE scheme_registered_under_care_act scheme_owning_organisation_name scheme_primary_client_group scheme_has_other_client_group scheme_secondary_client_group scheme_support_type scheme_intended_stay scheme_created_at]
      ...
      final_attributes = non_question_fields + attributes + scheme_attributes + location_attributes
